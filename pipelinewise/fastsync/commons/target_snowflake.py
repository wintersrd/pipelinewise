...
class FastSyncTargetSnowflake:
    ...
    def upload_to_s3(self, file, table):
        ...
        utils.log(
            "SNOWFLAKE - Uploading to S3 bucket: {}, local file: {}, S3 key: {}".format(
                bucket, file, s3_key
            )
        )
    ...
    def drop_table(self, target_schema, table_name, is_temporary=False):
        ...
        target_table = (
            table_dict.get("table_name") if not is_temporary else table_dict.get("temp_table_name")
        )
    ...
    def create_table(self, target_schema, table_name, columns, primary_key, is_temporary=False):
        ...
        if primary_key:
            ...
            , PRIMARY KEY ({}))
            """.format(
                target_schema, target_table, ", ".join(columns), primary_key
            )
        else:
            sql = """CREATE OR REPLACE TABLE {}.{} ({}
            ,_SDC_EXTRACTED_AT TIMESTAMP_NTZ
            ...
            """.format(
                target_schema, target_table, ", ".join(columns)
            )
        self.query(sql)
        ...
        for t in transformations:
            ...
            if t.get("tap_stream_name") == tap_stream_name_by_table_name:
                ...
                if transform_type == "SET-NULL":
                    ...
                    trans_cols.append(
                        "{} = CONCAT(SUBSTRING({}, 1, {}), SHA2(SUBSTRING({}, {} + 1), 256))".format(
    ...
    def swap_tables(self, schema, table_name):
        ...
        self.query(
            "ALTER TABLE {}.{} SWAP WITH {}.{}".format(schema, temp_table, schema, target_table)
        )
        self.query("DROP TABLE IF EXISTS {}.{}".format(schema, temp_table))

    def cache_information_schema_columns(self, tables):
        pipelinewise_schema = utils.tablename_to_dict(self.connection_config["stage"])[
            "schema_name"
        ]
        ...
        for schema_name in schemas_to_cache:
            ...
            """.format(
                    pipelinewise_schema, schema_name.lower()
                )
            )

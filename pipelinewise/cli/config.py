...
class Config(object):
    ...
    def from_yamls(cls, config_dir, yaml_dir=".", vault_secret=None):
        ...
        yamls = [
            ...
            if os.path.isfile(os.path.join(yaml_dir, f)) and f.endswith(".yml")
        ]
        ...
        for yaml_file in tap_yamls:
            ...
            if target_id not in targets:
                config.logger.error(
                    "Can't find the target with the ID \"{}\" but it's referenced in {}".format(
                        target_id, yaml_file
                    )
                )
                sys.exit(1)
    ...
    def save_tap_jsons(self, target, tap, extra_config_keys={}):
        ...
        tap_schema_mapping = {"schema_mapping": schema_mapping}

...
def test_get_bookmark_for_table_mysql():
    ...
    ) == {"replication_key": "date_updated", "replication_key_value": 123456, "version": 1}
...
def test_get_bookmark_for_table_postgresl():
    ...
    assert utils.get_bookmark_for_table(
        "public.city", postgres_properties, PostgresMock(), dbname="postgres_source_db"
    ...
    assert (
        utils.get_bookmark_for_table(
            "public.foo", postgres_properties, PostgresMock(), dbname="postgres_source_db"
        )
        == {}
    )

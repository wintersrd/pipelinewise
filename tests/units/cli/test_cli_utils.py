...
class TestUtils(object):
    ...
    def test_json_detectors(self):
        ...
        assert (
            cli.utils.is_json('{"prop-str":"dummy-string","prop-int":123,"prop-bool":true}') == True
        )

        assert cli.utils.is_json_file("./dummy-json") == False
        assert (
            cli.utils.is_json_file("{}/resources/example.json".format(os.path.dirname(__file__)))
            == True
        )
        assert (
            cli.utils.is_json_file("{}/resources/invalid.json".format(os.path.dirname(__file__)))
            == False
        )
    ...
    def test_json_loader(self):
        ...
        assert cli.utils.load_json(
            "{}/resources/example.json".format(os.path.dirname(__file__))
        ) == {
            "glossary": {
    ...
    def test_yaml_detectors(self):
        ...
        assert (
            cli.utils.is_yaml_file("{}/resources/example.yml".format(os.path.dirname(__file__)))
            == True
        )
        assert (
            cli.utils.is_yaml_file("{}/resources/invalid.yml".format(os.path.dirname(__file__)))
            == False
        )
    ...
    def test_yaml_loader(self):
        ...
        assert cli.utils.load_yaml(
            "{}/resources/example.yml".format(os.path.dirname(__file__))
    ...
    def test_sample_file_path(self):
        ...
        for sample in cli.utils.get_sample_file_paths():
            ...
            assert re.match(".*(tap|target)_.*.yml.sample$", sample) or re.match(
                ".*README.md$", sample
            )

    def test_extract_log_attributes(self):
        """Log files must match to certain pattern with embedded attributes in the file name"""
        assert cli.utils.extract_log_attributes(
            "snowflake-fx-20190508_000038.singer.log.success"
        ) == {
        ...
        assert cli.utils.extract_log_attributes(
            "snowflake-fx-20190508_231238.fastsync.log.running"
        ) == {
        ...
        assert cli.utils.extract_log_attributes("dummy-log-file.log") == {
    ...
    def test_fastsync_bin(self):
        ...
        assert cli.utils.get_fastsync_bin(
            VIRTUALENVS_DIR, "mysql", "snowflake"
    ...
    def test_vault(self):
        ...
        encrypted_str = str(
            cli.utils.vault_encrypt(
                "plain_text", "{}/resources/vault-secret.txt".format(os.path.dirname(__file__))
            )
        )
        ...
        assert (
            formatted_encrypted_str.startswith("!vault |")
            and "b'$ANSIBLE_VAULT;" in formatted_encrypted_str
        )

        # Optional name argument should add the name to the output string as a key
        formatted_encrypted_str = cli.utils.vault_format_ciphertext_yaml(
            encrypted_str, name="encrypted_plain_text"
        )
        assert (
            formatted_encrypted_str.startswith("encrypted_plain_text: !vault |")
            and "b'$ANSIBLE_VAULT;" in formatted_encrypted_str
        )
    ...
    def test_schema_loader(self):
        ...
        tap_schema = cli.utils.load_json(
            "{}/../../../pipelinewise/cli/schemas/tap.json".format(os.path.dirname(__file__))
        )
    ...
    def test_json_validate(self):
        ...
        valid_tap = cli.utils.load_yaml(
            "{}/resources/tap-valid-mysql.yml".format(os.path.dirname(__file__))
        )
        assert cli.utils.validate(valid_tap, schema) is None

        # Invalid instance should exit
        invalid_tap = cli.utils.load_yaml(
            "{}/resources/tap-invalid.yml".format(os.path.dirname(__file__))
        )
        with pytest.raises(SystemExit) as pytest_wrapped_e:
    ...
    def test_delete_keys(self):
        ...
        assert cli.utils.delete_empty_keys(
            {"foo": "bar", "foo2": None, "foo3": None, "foo4": "bar4"}
        ) == {"foo": "bar", "foo4": "bar4"}

        # Delete single key by name
        assert cli.utils.delete_keys_from_dict({"foo": "bar", "foo2": "bar2"}, ["foo2"]) == {
            "foo": "bar"
        }

        # Delete single key by name
        assert cli.utils.delete_keys_from_dict(
            {"foo": "bar", "foo2": "bar2", "foo3": None, "foo4": "bar4"}, ["foo2", "foo4"]
        ) == {"foo": "bar", "foo3": None}
        ...
        assert cli.utils.delete_keys_from_dict(
            ...
            ["foo2", "nested_foo"],
        ) == [{"foo": "bar"}, {"foo3": {"nested_foo2": "nested_bar2"}}]
    ...
    def test_tap_properties(self):
        ...
        assert (
            cli.utils.get_tap_stream_id(tap, "dummy_db", "dummy_schema", "dummy_table")
            == "dummy_schema-dummy_table"
        )

        # MySQL stream_name should be formatted as {{schema_name}}-{{table_name}}
        assert (
            cli.utils.get_tap_stream_name(tap, "dummy_db", "dummy_schema", "dummy_table")
            == "dummy_schema-dummy_table"
        )
        ...
        assert (
            cli.utils.get_tap_property_by_tap_type("tap-mysql", "default_replication_method")
            == "LOG_BASED"
        )
    ...
    def test_run_command(self):
        ...
        [rc, stdout, stderr] = cli.utils.run_command(
